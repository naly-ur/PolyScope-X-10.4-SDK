...
export default class extends Generator {
    ...
    writing() {
        ...
        for (let i = 0; i < dockerFilesToCopy.length; i++) {
            this.fs.copy(
                this.templatePath('../../docker-ros2/templates/' + dockerFilesToCopy[i]),
                this.destinationPath('../' + mainId + '/' + backendId + '/' + dockerFilesToCopy[i])
            );
        }
        copyTpl('docker-ros2/package.json', 'package.json', mainId, backendId);
        copyTpl('../../docker-ros2/templates/Dockerfile','Dockerfile', mainId, backendId);
        const docSrc = '../../docker-ros2/templates/src/ros2_node/';
        const docDst = 'src/' +  ros2NodeName + '/';
        copyTpl(docSrc + 'resource/ros2_node', docDst + '/resource/' + ros2NodeName, mainId, backendId);
        copyTpl(docSrc + 'ros2_node/__init__.py', docDst + '/' + ros2NodeName + '/__init__.py', mainId, backendId);
        copyTpl(docSrc + 'ros2_node/main.py', docDst + '/' + ros2NodeName + '/main.py', mainId, backendId);
        copyTpl(docSrc + 'package.xml', docDst + '/package.xml', mainId, backendId);
        copyTpl(docSrc + 'setup.cfg', docDst + '/setup.cfg', mainId, backendId);
        copyTpl(docSrc + 'setup.py', docDst + '/setup.py', mainId, backendId);
    }
};
